...
if (!B24_WEBHOOK) {
    console.warn('[WARN] B24_WEBHOOK is not set — /book and /event will fail until it is configured.');
}
...
function firstArray(obj) {
    ...
    if (obj && typeof obj === 'object') for (const k in obj) if (Array.isArray(obj[k])) return obj[k];
    return [];
}
...
async function getStatusEnum() {
    ...
    const list = (res && res.productPropertyEnums) ? res.productPropertyEnums : firstArray(res);
    const byLabel = {}, byId = {};
...
async function getPipeline() {
    ...
    const match = clist.find(function (c) {
        return String(c.name || c.NAME || '').trim().toLowerCase() === PIPELINE_NAME.toLowerCase();
    }) || clist.find(function (c) {
        return String(c.name || c.NAME || '').toLowerCase().indexOf(PIPELINE_NAME.toLowerCase()) !== -1;
    });
    ...
    const stages = await callWebhook('crm.status.list', { order: { SORT: 'ASC' }, filter: { ENTITY_ID: entityId } });
...
function extractPropRaw(product, propId) {
    const keys = ['property' + propId, 'PROPERTY_' + propId, 'property_' + propId, String(propId)];
    for (let i = 0; i < keys.length; i++) {
        let v = product[keys[i]];
        if (v === undefined || v === null) continue;
        if (Array.isArray(v)) v = v[0];
        if (v && typeof v === 'object') {
            v = (v.value !== undefined ? v.value : (v.valueId !== undefined ? v.valueId : v.VALUE));
        }
        return v;
    }
    return null;
...
async function setUnitStatus(productId, label) {
    ...
    if (!enumId) {
        throw new Error('Status "' + label + '" not in PROPERTY_' + STATUS_PROPERTY_ID +
            ' enum (found: ' + Object.keys(en.byLabel).join(', ') + ')');
    }
    const fields = {};
    fields['property' + STATUS_PROPERTY_ID] = enumId;
    await callWebhook('catalog.product.update', { id: productId, fields: fields });
    return enumId;
}
...
app.post('/book', async (req, res) => {
    try {
        ...
        if (!dealId || !productId) return res.status(400).json({ ok: false, error: 'dealId and productId required' });
...
app.post('/event', (req, res) => {
    ...
    processDealEvent(req.body).catch(function (e) { console.error('[/event] error:', e.message); });
});

async function processDealEvent(body) {
    body = body || {};
    if (String(body.event || '').toUpperCase() !== 'ONCRMDEALUPDATE') return;
    if (B24_EXPECTED_DOMAIN && body.auth && body.auth.domain && body.auth.domain !== B24_EXPECTED_DOMAIN) {
    ...
    const productId = rlist.length ? (rlist[0].PRODUCT_ID || rlist[0].productId) : null;
    if (!productId) return;
    ...
    console.log('[/event] deal ' + dealId + ' @ "' + stageRec.name + '" → unit ' + productId + ' = ' + target);
}
